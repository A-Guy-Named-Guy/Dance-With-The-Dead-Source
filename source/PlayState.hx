...
class PlayState extends MusicBeatState
{
	...
	var santa:FlxSprite;
	var bgCrows:FlxSprite;
	var crowFly:FlxSprite;
				...
				case 'graveyard':
					{
						defaultCamZoom = 0.8;
						curStage = 'graveyard';
						var bg:FlxSprite = new FlxSprite(-540, -200).loadGraphic(Paths.image('graveyard_sky'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.5, 0.5);
						bg.active = false;
						add(bg);

						var background:FlxSprite = new FlxSprite(-540, -200).loadGraphic(Paths.image('graveyard_background'));
						background.antialiasing = true;
						background.scrollFactor.set(0.7, 0.7);
						background.active = false;
						add(background);

						var stageFront:FlxSprite = new FlxSprite(-540, -200).loadGraphic(Paths.image('graveyard_foreground'));
						...
						// var stageCurtains:FlxSprite = new FlxSprite(-540, -200).loadGraphic(Paths.image('graveyard_crows'));
						...
						crowFly = new FlxSprite(-540, -200);
						crowFly.updateHitbox();
						crowFly.antialiasing = true;
						crowFly.frames = Paths.getSparrowAtlas('crowfly');
						crowFly.animation.addByPrefix('fly', "crowfly idle", 12);
						crowFly.animation.play('fly');
						crowFly.scrollFactor.set(1, 1);

						bgCrows = new FlxSprite(-540, -200);
						bgCrows.updateHitbox();
						bgCrows.antialiasing = true;
						bgCrows.frames = Paths.getSparrowAtlas('graveyard_crows');
						bgCrows.animation.addByPrefix('idle', "graveyard_crows_idle", 24);
						bgCrows.animation.play('idle');
						bgCrows.scrollFactor.set(1, 1);

						if (SONG.song.toLowerCase() == 'thrill')
						{
							add(crowFly);
							new FlxTimer().start(0.1, function(flyTimer:FlxTimer)
							{
								FlxG.sound.play(Paths.sound('crowcall'));
							});
							{
								new FlxTimer().start(2, function(flyTimer:FlxTimer)
								{
									remove(crowFly);
									add(bgCrows);
								});
						...
						else if (SONG.song.toLowerCase() != 'thrill')
						{
							add(bgCrows);
						}
					}
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'roses'
			...
			|| StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'thrill')
		{
			remove(black);
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					else if (SONG.song.toLowerCase() == 'outro')
					{
						add(crowFly);
						FlxG.sound.play(Paths.sound('crowcall'));
						{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					//	}
					//  Old transition from in a week to the main menu. New if statement is below to allow a video to play after the week ends

					if (FlxG.save.data.scoreScreen)
						openSubState(new ResultsScreen());
					else if (curSong == 'vigor' && !isCutscene)
					{
					...
					else
					{
						...
						if (isCutscene && playingCutscene == false)
						{
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'graveyard':
				if (FlxG.save.data.distractions)
				{
					bgCrows.animation.play('idle', true);
				}
